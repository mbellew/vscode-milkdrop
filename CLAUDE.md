...
- **Validate expression-language changes against the real compiler.** projectm-eval is vendored at `../projectm/vendor/projectm-eval/` and built to `../projectm/cmake-build/vendor/projectm-eval/projectm-eval/libprojectM_eval.a`. Build a tiny C oracle that calls `projectm_eval_code_compile()` (NULL return = compile failure) — supply no-op `projectm_eval_memory_host_lock_mutex`/`unlock_mutex` stubs, include `vendor/projectm-eval/projectm-eval/api/projectm-eval.h`, link the `.a` with `-lm`. Feed it the same reassembled block source `expr.ts` parses and diff the accept/reject verdicts (temporarily re-export `parseSource` from `expr.ts` to drive it). `expr.ts` was confirmed at 100% agreement (0 FP / 0 FN) over all 66,076 blocks this way; rerun it after any tokenizer/grammar edit. Note: GetCode joins lines with bare `\n` (no `;` inserted — see `PresetFileParser.cpp:101`), so a line that forms a *complete* statement with no trailing `;` followed by another statement is a genuine error, while a line ending mid-expression (e.g. trailing `*`) legally continues onto the next (`presets/tests/104-continued-eqn.milk`).
